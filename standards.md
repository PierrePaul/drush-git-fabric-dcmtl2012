...
## Standards
![Copyright Claudine](images/standards/bling-bling.gif)
...
Dans le cas où on aurait un bloc d'appel, il faut aligner les signes égal...
    ...
    $impot_python             = calcul_impot(10);

Pour la déclaration d'une fonction, un espace après les paramètres :

    function calcul_impot_python ($salaire, $taux_imposition = 0.1) {
    }

## Appel à un constructeur
Toujours inclure les parenthèses, même s'il n'y a aucun argument:
...
C'est fortement recommandé de garder une virgule après le dernier item.
...
Les paramètres devraient être énumérés et expliqués.
...
Utilisation de `check_plain`, `filter_xss` sur le output.

## Drupal sandbox with Coder 
...
## Migrate
Jamais nécessaire d'avoir un upgrade path pour le code. Question de bon sens et de confiance en ce qui concerne le data.

## Examples module
<http://drupal.org/project/examples>

Contient des exemples de tests pour (presque) chacun des API fournies par le core de Drupal.
